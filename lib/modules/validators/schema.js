...
prototype.addValidator = function(fieldName, validatorData) {
  ...
  if (!_.isObject(validatorData)) {
    throw new Error('Provide validation data for `' + fieldName +
      '` field in `' + this.getName() + '` class schema');
  }
  if (!_.has(validatorData, 'type')) {
    throw new Error('Provide validator type for `' + fieldName +
      '` field in `' + this.getName() + '` class schema');
  }

  // Check whether given validator type exists.
  if (!_.has(Validators, validatorData.type)) {
    throw new Error('Validator type `' + validatorData.type +
      '` provided for `' + fieldName + '` field in `' + this.getName() +
  ...
  if (!_.isObject(validatorData.options)) {
    throw new Error('Validator options for `' + fieldName +
      '` field in `' + this.getName() + '` class schema have to be object');
  }
  if (!_.isObject(validatorData.messages)) {
    throw new Error('Validator messages for `' + fieldName +
      '` field in `' + this.getName() + '` class schema have to be object');
  }
  ...
  if (_.has(validator, 'requiredOptions')) {
    ...
    if (!hasAll) {
      throw new Error('Provide all required validator options for `' +
        fieldName + '` field in `' + this.getName() + '` class schema');
    }
  }
...
prototype.addValidators = function(validatorsData) {
  if (!_.isObject(validatorsData)) {
    throw new Error('Validation data in `' + this.getName() +
  ...
  _.each(validatorsData, function(validatorData, fieldName) {
    if (_.isArray(validatorData)) {
      ...
      throw new Error('Validation data for `' + fieldName +
        '` field in `' + this.getName() +
        '` class schema has to be an object or array');
    }
  }, this);
};
