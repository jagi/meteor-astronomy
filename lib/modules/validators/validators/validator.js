Astro.Validator = class Validator {
  ...
  static create(definition) {
    let validator = new Astro.Validator(definition);
    Astro.Validator.validators[validator.name] = validator;
    Validators[validator.name] = function({
      ...
      if (resolveParam instanceof Function) {
        param = resolveParam({ doc, name, value });
      }
      // Parse param type.
      ...
      if (!validator.isValid(args)) {
        ...
        let throwError = function(error) {
          ...
          throw new Astro.ValidationError([{
            type: validator.name, name, value, param, error
          }], error);
        };
        ...
        throwError(Astro.ValidationError.DEFAULT_REASON);
      }
    };
  }
};