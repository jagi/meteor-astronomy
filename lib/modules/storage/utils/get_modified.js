Astro.utils.storage.getModified = function(currDoc, old, options) {
  old = old || false;
  ...
  let rawCurrDoc = Astro.utils.fields.rawAll(currDoc, options);
  let rawPrevDoc = Astro.utils.fields.rawAll(prevDoc, options);
      ...
      if (
        ...
        for (let i = 0; i < currFieldValue.length; i++) {
          ...
          if (i < prevFieldValue.length) {
            // If array elements are object, then we perform diff between them.
            if (_.isObject(currElement) && _.isObject(prevElement)) {
            ...
            // If array elements are plain types, then we compare them.
