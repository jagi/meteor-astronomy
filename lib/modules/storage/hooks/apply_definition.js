...
from '../class_static_methods/find';
...
import protoGetModifier from '../class_prototype_methods/get_modifier';
import protoGetModified from '../class_prototype_methods/get_modified';
import protoGetModifiedValues from '../class_prototype_methods/get_modified_values';
import protoIsModified from '../class_prototype_methods/is_modified';
// Meteor methods.
...
// Class events.
import beforeInit from '../class_events/before_init';
import afterInit from '../class_events/after_init';
import fromJSONValue from '../class_events/from_json_value';
import toJSONValue from '../class_events/to_json_value';
...
function onApplyDefinition(Class, parsedDefinition, className) {
  ...
  if (Collection) {
    ...
    // function.
    if (isFunction(parsedDefinition.transform)) {
      schema.transform = function(rawDoc) {
        return parsedDefinition.transform(resolveValues({
          Class,
          rawDoc
        }));
      };
    ...
    if (parsedDefinition.secured !== undefined) {
      extend(schema.secured, parsedDefinition.secured);
    }
  }
};
