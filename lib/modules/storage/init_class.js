  ...
  docs.forEach(function(doc) {
    ...
    if (modifier.$push) {
      _.each(modifier.$push, function(fieldValue, fieldName) {
        if (_.isArray(fieldValue.$each)) {
          _.each(fieldValue.$each, function(element) {
    ...
    // Get a modifier with casted values and prepared for save.
    modifier = doc._getModifier();

    console.log('modifier', modifier);

    // Execute the update method.
    Collection.update(doc._id, modifier, _.omit(options, 'multi'), callback);
  });
};
