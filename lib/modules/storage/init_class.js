var storageClassPrototypeMethods = {
...
Astro.eventManager.on('initClass', function(schemaDefinition) {
  ...
  _.extend(Class.prototype, storageClassPrototypeMethods);
  ...
  if (typeField) {
    ...
    Class.addEvent('initInstance', function(attrs) {
      console.log('initInstance attrs:', attrs);
      var doc = this;
  ...
  if (collectionInfo) {
    collectionInfo.classes.push(Class.getName());
    if (!collectionInfo.typeField) {
      collectionInfo.typeField = typeField || 'type';
    }
  } else {
    collectionInfo = Astro.collections[Collection._name] = {
