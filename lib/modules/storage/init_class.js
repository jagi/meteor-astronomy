...
prototypeMethods.reload = function() {
  ...
  if (doc._id) {
    ...
    var plainDoc = Collection.findOne(id, {
      transform: null,
    });
...
var checkSelector = function(selector, methodName) {
  ...
  if (
    ...
    throw new Meteor.Error(
      403,
      'Not permitted. Untrusted code may only ' + operation +
      ' documents by ID.'
    );
  }
};
 ...
 * @returns {Number} Returns number of updated documents.
 */
classMethods.update = function(selector, modifier, options, callback) {
  ...
  try {
    ...
    // If there are no matching document and the "upsert" option was set, then
    // we have to insert a new document.
    if (doc.count() === 0 && options.upsert) {
 ...
 * @param {Boolean} options.multi True to modify all matching documents; false to only modify one of the matching documents (the default).
 * @param {Function} [callback] Optional. If present, called with an error object as the first argument and, if no error, the number of affected documents as the second.
 * @returns {Object} Returns an object with keys `numberAffected` (the number of documents modified)  and `insertedId` (the unique _id of the document that was inserted, if any).
 */
