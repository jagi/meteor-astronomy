...
methods._pushValue = function(fieldName, fieldValue, options) {
  var doc = this;
...
methods._pushOne = function(fieldName, fieldValue) {
  var doc = this;
  ...
  doc._pushValue(fieldName, fieldValue);
...
methods._pushArray = function(fieldName, fieldValues) {
  var doc = this;

  _.each(fieldValues, function(fieldValue) {
    doc._pushOne(fieldName, fieldValue);
  });
};

methods._pushMany = function(fieldsValues) {
  var doc = this;

  // Set multiple fields.
  _.each(fieldsValues, function(fieldValue, fieldName) {
    if (_.isArray(fieldValue)) {
      doc._pushArray(fieldName, fieldValue);
    } else {
      doc._pushOne(fieldName, fieldValue);
    }
  });
