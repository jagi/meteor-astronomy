...
var setOne = function(fieldName, value) {
  ...
  // If value differs from the old value then set the new one.
  if (value !== this._values[fieldName]) {

    console.log(1, value, this._values[fieldName]);
    this._modified[fieldName] = value;

    // If value differs from the modified one and we go back to the old value...
  } else if (value !== this._modified[fieldName]) {

    // ... then we can remove information that given field had been modified.
    console.log(2, value, this._values[fieldName]);
    delete this._modified[fieldName];
