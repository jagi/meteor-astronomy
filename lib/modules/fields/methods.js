...
var getOne = function(fieldName) {
  ...
  if (_.has(this._modified, fieldName)) {
    ...
    // then we have to look for default value in the field's definition. We have
    // to use EJSON stringify and EJSON parse methods to make sure that set
    // value is a copy of default value and not a reference. At this stage it's
    // important to have defined custom types for EJSON thanks to which it will
    // be possible to clone any non standard value.
    value = EJSON.parse(EJSON.stringify(fieldDefinition.default));
