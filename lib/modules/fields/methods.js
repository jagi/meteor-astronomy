...
methods.save = function(callback) {
  ...
  // Remove "_id" field, if its value is null or we are updating document.
...
var getOne = function(fieldName) {
  ...
  if (_.has(this._modified, fieldName)) {
    ...
    // then we have to look for default value. We already have field's
    // definition, so we only have to set value.
