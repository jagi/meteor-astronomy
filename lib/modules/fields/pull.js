  ...
  Astro.utils.fields.traverseNestedDocs(
    ...
    function(nestedDoc, nestedFieldName, Class, field, index) {
      ...
      if (field) {
        ...
        if (options.modifier) {
          modifierPulledValue = field.plain(EJSON.clone(pullValue));
        }
      } else if (Class) {
        Astro.utils.warn(
      ...
      if (nestedDoc[nestedFieldName].indexOf(pullValue) === -1) {
        return;
      }
      ...
      var modifiedArray = [];
      _.each(nestedDoc[nestedFieldName], function(value) {
        if (!EJSON.equals(value, pullValue)) {
          modifiedArray.push(value);
        }
      });
      nestedDoc[nestedFieldName] = modifiedArray;
    }
  );
...
proto.pull = function(/* arguments */) {
  ...
  if (args.length === 1 && _.isObject(args[0])) {
    doc._pullMany(args[0]);
  } else if (args.length === 2) {
    doc._pullOne(args[0], args[1]);
  }
};
