Astro.utils.fields = {
  ...
  getDefaultValue: function(Class, fieldNameOrPattern) {
    ...
    console.log('getDefaultValue', fieldNameOrPattern);
    ...
    if (_.isUndefined(value) && self.isPattern(fieldNameOrPattern)) {
      // Split the pattern into segments.
      var segments = fieldNameOrPattern.split('.');

      console.log('STILL UNDEFINED', segments);
    }
  ...
  castValue: function(Class, fieldNameOrPattern, value) {
    ...
    if (fieldDefinition) {
      return Astro.utils.types.castValue(fieldDefinition.type, value);
    }
