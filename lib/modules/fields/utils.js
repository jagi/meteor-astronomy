Astro.utils.fields = {
  ...
  getDefaultValue: function(Class, fieldName) {
    ...
    var field = self.getDefinition(Class, fieldName);
  ...
  castValue: function(Class, fieldName, value) {
    ...
    if (fieldDefinition) {
      ...
      return fieldDefinition.cast(value, fieldDefinition);
    }
  ...
  getPlainValue: function(Class, fieldName, value) {
    console.log('utils.getPlainValue');
    var self = this;
    ...
    if (fieldDefinition) {
      ...
      return fieldDefinition.plain(value, fieldDefinition);
    }
        ...
        if (Class) {
          ...
          if (fieldDefinition) {
            ...
            if (_.isUndefined(fieldValue) && options.default) {
              // ... then try getting a default value.
              fieldValue = fieldDefinition.getDefault();
            } else if (options.cast) {
              // Try casting the value to the proper type.
              fieldValue = fieldDefinition.cast(fieldValue);
            }
          } else {
            console.warn(
