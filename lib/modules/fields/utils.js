Astro.utils.fields = {
  ...
  getModifiedValues: function(doc, old, options) {
    ...
    _.each(fieldsValues, function(fieldValue, fieldName) {
      ...
      if (!EJSON.equals(doc._original[fieldName], fieldValue)) {
        ...
        // If we want plain objects instead of nested Astronomy documents, then
        // the "plain" flag will get plain data from each document.
        if (options.plain && value instanceof Astro.base.Class) {
          value = self.getAllValues(value, options);
        }
  ...
  getValue: function(doc, fieldName, options) {
    ...
    // Setting the "plain" flag will get a plain data from each nested document
    // instead of Astronomy documents.
    if (options.plain && value instanceof Astro.base.Class) {
      value = Astro.utils.fields.getAllValues(value, options);
    }
