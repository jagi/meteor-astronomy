var save = function(callback) {
  ...
  // Check if there are any values that have relations defined. If do, then try
  // saving them in their collections.
  _.each(values, function(value, fieldName) {
    // If the value is not object or array then we can stop trying to save
    // related object.
    ...
    var schema = _.find(this.constructor.schemas, function(schema) {
      return schema.hasRelation(fieldName);
    });

    if (schema) {
      var relation = schema.getRelation(fieldName);
      console.log(
        'set foreign',
        relation.foreign,
        'with local',
        relation.local,
        this.get(relation.local)
      );
      value.set(relation.foreign, this.get(relation.local));
      delete values[fieldName];
      value.save();
    }
  }, this);

  // One more time check if there are any fields to save. After saving related
  // fields we could get empty the "values" object.
  if (_.size(values) > 0) {
