...
import cloneDefinition from '../modules/core/utils/clone_definition';
...
class Class {
  constructor(rawDoc = {}, options = {}) {
    ...
    // In this place, we have to directly set the "_id" field so that in the
    // "beforeInit" event it will be possible to check if a document is new or
    ...
    each(Class.getFieldsNames(), (fieldName) => {
  ...
  static extend(extendDefinition, onlyModules) {
    ...
    Module.forEach((module) => {
      ...
      if (onlyModules !== undefined && !includes(onlyModules, module.name)) {
        return;
      }
  ...
  static includes(Class) {
    return includes(this.classes, Class);
  }
  ...
  static isNew(doc) {
    if (!doc._id) {
      ...
      return this.find(doc._id).count() === 0;
    }
  }
};
